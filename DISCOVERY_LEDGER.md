...
5. **Mode 0 Unfurl Geometry & Intermediate Trajectory Calibration**:
   ...
   - **Resolved Polar Point Buckling & Layer Inversion**:
     ...
     - *3D Sticker Peel Model Calibration*: Formulated and deployed the 3D Sticker Peel kinematics. Replaced the sluggish cubic onset with an immediate power-law onset ($t^{0.78}$), enabling the antimeridian seam to visibly crack and roll outward by 50–60 pixels from $\alpha = 0.02–0.05$ with a distinct fingernail lip. Propagates the traveling peel line from 1.0 down to 0.0, keeping unpeeled longitudes at full spherical curvature and completely eliminating premature whole-globe deflation ("taking a breath in").
