...
async function verify() {
  ...
  await page.waitForFunction(
    () => {
      const eng = (window as any).__INDICATRIX_WEBGPU_ENGINE__ || (window as any).__ENGINE;
  ...
  await page.evaluate(() => {
    (window as any).__INDICATRIX_SCRUB_ALPHA__ = 0.42;
  });
  await page.waitForTimeout(100);
  const scrubVal = await page.evaluate(() => (window as any).__INDICATRIX_SCRUB_ALPHA__);
  if (scrubVal !== 0.42) {
  ...
  const results: Array<{ mode: string; alpha: number; file: string; size: number }> = [];
    ...
    if (modeBtn) {
      ...
      await page.evaluate((modeId) => {
        if ((window as any).__INDICATRIX_ENGINE__) {
          (window as any).__INDICATRIX_ENGINE__.setMode(modeId);
        }
      }, m.id);
    }
