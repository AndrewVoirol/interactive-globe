...
export function evaluateManifoldCore(
  ...
  switch (mode) {
    ...
    default: {
      ...
      const alpha = Math.max(0.0, Math.min(1.0, unfurl));
      const lonRad = (Math.abs(mercator2D[0]) > 0.00001 || Math.abs(mercator2D[1]) > 0.00001)
        ? mercator2D[0] / RADIUS
        : Math.atan2(pos3D[0], pos3D[2]);
      ...
      outNormal = nLen > 0.00001 ? [rawNx / nLen, rawNy / nLen, rawNz / nLen] : [0.0, 0.0, 1.0];
      break;
    }
  }
...
describe('Challenger Phase 2.3: evaluateManifold Unification Stress Harness', () => {
  ...
  describe('Pillar 5: Normal Vector Normalization Empirical Analysis', () => {
    ...
    it('CHALLENGE-2.3-11: Confirms normal collapse is eliminated in Mode 0 at antimeridian equator when alpha=0.5', () => {
      ...
      // With calibrated 4% chiral waveBias (normLon * 0.04 * env), uAngle ≈ 1.696 rad at lambda = PI:
      ...
      // Confirms normal length is strictly unit normalized (>= 0.99) and oriented along tangent [0.992, 0, -0.125]
      expect(lenMode0).toBeGreaterThanOrEqual(0.99);
      expect(resMode0.normal[0]).toBeCloseTo(0.992, 2);
      expect(resMode0.normal[1]).toBeCloseTo(0.0, 2);
      expect(resMode0.normal[2]).toBeCloseTo(-0.125, 2);
    });
  });
  ...
  describe('Pillar 7: Mode 0 Perfect Kinematics & Mathematical Acceptance Criteria', () => {
    ...
    it('CHALLENGE-2.3-16: Seam-confined tactile lip detachment: crack opens immediately by alpha=0.05 with outward radial curl and asymmetric chiral peeling', () => {
      // At alpha = 0.05:
      // Seam zone (|lon| = 175° > 90°, absNormLon = 175/180 = 0.972):
      const westSeam = evaluateManifoldCore(geoCoords(-175, 0).pos3D, geoCoords(-175, 0).mercator2D, 0.05, 0);
      const eastSeam = evaluateManifoldCore(geoCoords(175, 0).pos3D, geoCoords(175, 0).mercator2D, 0.05, 0);

      // Chiral seam opening: West flap curls outward away from origin (more negative Z, < -4.981)
      // while East flap tucks inward toward origin (less negative Z, > -4.981)
      expect(westSeam.pos[2]).toBeLessThan(-4.981);
      expect(eastSeam.pos[2]).toBeGreaterThan(-4.981);
      // Confirms seam crack opens immediately at alpha=0.05 with observable Z-depth separation (> 0.20):
      expect(Math.abs(westSeam.pos[2] - eastSeam.pos[2])).toBeGreaterThan(0.20);

      // Inner continental longitudes (|lon| <= 90°):
      // seamZone and microRim are strictly 0.0, ensuring zero seam lip disturbance
      const innerLons = [-90, -45, 0, 45, 90];
      for (const lon of innerLons) {
        const coords = geoCoords(lon, 0);
        for (const alpha of [0.05, 0.25, 0.50, 0.75]) {
          const res = evaluateManifoldCore(coords.pos3D, coords.mercator2D, alpha, 0);
          const lonRad = (lon * Math.PI) / 180;
          const normLon = lonRad / Math.PI;
          const sZero = smoothstep(0.0, 0.10, alpha);
          const sOne = 1.0 - smoothstep(0.90, 1.0, alpha);
          const env = Math.sin(Math.PI * alpha) * sZero * sOne;
          const alphaEased = alpha * alpha * (3.0 - 2.0 * alpha);
          const waveBias = normLon * 0.040 * env;
          const easeLocal = Math.max(0.0, Math.min(1.0, alphaEased - waveBias));
          const sBase = Math.max(0.0, 1.0 - easeLocal);
          const cosHalfLon = Math.cos(lonRad * 0.5);
          const spineWeight = Math.pow(cosHalfLon, 4);
          const sLocal = sBase * (1.0 - 0.10 * env * spineWeight);
          const uAngle = sLocal * lonRad;
          const expectedX = Math.abs(uAngle) > 0.02
            ? RADIUS * (Math.sin(uAngle) / Math.max(0.0001, sLocal))
            : RADIUS * lonRad * (1.0 - uAngle * uAngle / 6.0);
    ...
    it('CHALLENGE-2.3-17: Polar apex convergence & flat neatline landing', () => {
      ...
      // Invariant: Zero polar tearing at alpha <= 0.05 (watertight apex across all longitudes)
      for (const poleLat of [90.0, -90.0]) {
        for (const alpha of [0.0, 0.05]) {
          const ref = evaluateManifoldCore(geoCoords(0, poleLat).pos3D, geoCoords(0, poleLat).mercator2D, alpha, 0);
          for (const lon of [-180, -120, -60, 60, 120, 180]) {
            const pt = evaluateManifoldCore(geoCoords(lon, poleLat).pos3D, geoCoords(lon, poleLat).mercator2D, alpha, 0);
            const apexDelta = Math.hypot(pt.pos[0] - ref.pos[0], pt.pos[1] - ref.pos[1], pt.pos[2] - ref.pos[2]);
            expect(apexDelta).toBeCloseTo(0.0, 5);
          }
        }
      }
      ...
      // pos must exactly match 2D Equirectangular sheet [R * lonRad, R * latRad, 0]
      for (const lon of [-180, -90, 0, 90, 180]) {
        for (const lat of [-90, -85, -60, -30, 0, 30, 60, 85, 90]) {
          ...
          const expectedX = (lon * Math.PI / 180.0) * RADIUS;
          const expectedY = (lat * Math.PI / 180.0) * RADIUS;
          expect(res1.pos[0]).toBeCloseTo(expectedX, 4);
          expect(res1.pos[1]).toBeCloseTo(expectedY, 4);
    ...
    it('CHALLENGE-2.3-18: Developable arc kinematics & depth preservation', () => {
      ...
      for (const alpha of [0.25, 0.50, 0.75]) {
        ...
        // Developable arc depth is strictly preserved above the flat drafting board (Z > 0)
        expect(res.pos[2]).toBeGreaterThan(0.0);
        if (alpha === 0.50) {
          expect(res.pos[2]).toBeCloseTo(2.58, 1);
        }
      }
    });
  });
